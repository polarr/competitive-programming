...
struct InfoSum {
	ll sum = 0;
	void apply(const TagSum &t, int l, int r) {
		if (t.type == SET) {
			sum = t.val * (r - l + 1);
		} else if (t.type == ADD) {
			sum += t.val * (r - l + 1);
...
/** @return result of joining nodes a and b together */
InfoSum operator+(const InfoSum &a, const InfoSum &b) { return {a.sum + b.sum}; }

void solve(){
    int n, q; cin >> n >> q;
    vector<InfoSum> arr(n); 
    rep(i, 0, n) {
        ll a; cin >> a;
        arr[i] = {a};
    }
    ...
    LazySegtree<Info, Tag> st(s);
    LazySegtree<InfoSum, TagSum> st2(arr);
    rep(i, 0, q) {
        int c; cin >> c;
        int a, b; cin >> a >> b;
        --a; --b;
        if (c == 1) {
            st.range_update(a, b, Tag{ADD, 1});
            st2.range_update(a, b, TagSum{ADD, -a});
        } else {
            cout << st.range_query(a, b).val[0] + st2.range_query(a, b).sum << '\n';
        }
    }
}
