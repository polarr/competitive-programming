 ...
 * PURPOSE: Uses shoelace theorem to calculate polygon area. Remove abs for signed area.
 * CONSTRAINT: Point locations <= 1e9 to avoid overflow
*/
ll polygon_area(const vector<Point<ll>> &points) {
    int n = sz(points);
    ll ans = 0;
    rep(i, 0, n) {
        int pv = (i + n - 1) % n;
...
void solve(){
    ...
    ll ans = polygon_area(p) + 2;
    ll count = 0;

    rep(i, 0, n) {
        int j = (i + 1) % n;
        ll a = abs(p[i].x - p[j].x);
        ll b = abs(p[i].y - p[j].y);
