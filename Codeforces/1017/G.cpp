...
struct Info {
	pii val = {INT_MIN, INT_MIN};
	void apply(const Tag &t, int l, int r) {
		if (t.type == ADD){
            val.first += t.val;
            val.second += t.val;
        } else if (t.type == SET){
            val = {t.val * (r - l + 1), t.val};
        }
	}
};
...
Info operator+(const Info &a, const Info &b) {
    if (a.val.first == INT_MIN){
        return b;
    }
    if (b.val.first == INT_MIN){
        return a;
    }

    return Info{{a.val.first + b.val.first, max(a.val.second + b.val.first, b.val.second)}};
}
...
template <class T, bool VALS_IN_EDGES> class HLD {
  public:
	...
	int query_path(int u, int v) {
		int res = INT_MIN;  // default value, modify depending on problem
        int curr = 0;
		process(u, v, [&](int l, int r) {
            pii qr = segtree.range_query(l, r).val;
            res = max(res, curr + qr.second);
            curr += qr.first;
		});
		return res;
	}

	void modify_node(int u, T val) { segtree.range_update(pos[u], pos[u], Tag{ADD, val}); }
    void modify_subtree(int u, T val){
        segtree.range_update(pos[u], epos[u], Tag{SET, val});
    }
};
...
void solve(){
    ...
    rep(i, 0, q){
        ...
        if (c == 1){
            ...
            hld.modify_subtree(v, -1);
            int qr = hld.query_path(0, v);
            if (qr < 0) continue;
            hld.modify_node(v, -qr - 1);
        } else {
            int qr = hld.query_path(0, v);
            if (qr >= 0){
                cout << "black" << endl;
