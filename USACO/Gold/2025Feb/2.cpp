...
class CompressedBinary {
    public: 
        ...
        CompressedBinary(int nn, set<int>& updates){
            n = nn;
            int prev = 0;
            int val = 0;
            int i = 0;
            ...
            for (int x : updates){
                ...
                if (val == 1){
                    ones += x - prev;
                    s.pb({ones + n - (x - 1), prev, x - 1});
                }
                blocks.pb({i++, prev, x - 1, val});
                prev = x;
                val ^= 1;
            }
            ...
            blocks.pb({prev, n, val});
        }

        int search(int x){
            auto j = upper_bound(all(s), x);
        }
...
void solve(){
    ...
    rep(i, 0, m){
        int l, r; cin >> l >> r;
        ++r;
        if (updates.contains(l)){
            updates.erase(l);
        } else {
            updates.insert(l);
        }

        if (updates.contains(r)){
            updates.erase(r);
        } else {
            updates.insert(r);
        }
